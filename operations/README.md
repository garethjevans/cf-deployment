...
| **GCP** | | |
| [`use-gcs-blobstore.yml`](use-gcs-blobstore.yml) | Replaces local WebDAV blobstore with external GCS Storage blobstore in interoperability mode. | Introduces [new variables](example-vars-files/vars-use-gcs-blobstore.yml) for GCS credentials and container names. |
| **Openstack** | | |
